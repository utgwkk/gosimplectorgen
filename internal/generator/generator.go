...
func (g *Generator) Generate() error {
	...
	writeHeader(&buf, f.Name.Name)
	writeImports(&buf, f.Imports, structs)
	writeConstructors(&buf, fset, src, structs)
...
// writeHeader emits the "do not edit" banner and the package declaration.
...
// writeImports emits an import block containing only the packages actually
// referenced by the collected struct field types, preserving any alias names
// from the source file.
func writeImports(buf *bytes.Buffer, fileImports []*ast.ImportSpec, structs []structInfo) {
...
// writeConstructors emits a constructor function for each collected struct.
func writeConstructors(buf *bytes.Buffer, fset *token.FileSet, src []byte, structs []structInfo) {
	for _, s := range structs {
		writeConstructor(buf, fset, src, s)
	}
}

// writeConstructor emits a single constructor function that accepts one
// parameter per field and returns a pointer to a fully initialised struct.
func writeConstructor(buf *bytes.Buffer, fset *token.FileSet, src []byte, s structInfo) {
